...
trait Util[Builder, Output <: FragT, FragT] extends LowPriUtil[Builder, Output, FragT] {
   ...
   */
  implicit class SeqNode[A](xs: Seq[A])(implicit ev: A => Modifier[Builder])
...
trait LowPriUtil[Builder, Output <: FragT, FragT] {
   ...
   * Renders an Seq of [[FragT]] into a single [[FragT]]
   */
  implicit def GeneratorFrag[A](xs: geny.Generator[A])(implicit
      ev: A => Frag[Builder, FragT]
  ): Frag[Builder, FragT]

  /**
   * Renders an Option of [[FragT]] into a single [[FragT]]
   */
  implicit def OptionFrag[A](xs: Option[A])(implicit
      ev: A => Frag[Builder, FragT]
  ): Frag[Builder, FragT] = SeqFrag(xs.toSeq)

  /**
   * Renders an Seq of [[FragT]] into a single [[FragT]]
   */
  implicit def ArrayFrag[A](xs: Array[A])(implicit
      ev: A => Frag[Builder, FragT]
  ): Frag[Builder, FragT] = SeqFrag[A](xs.toSeq)
